...
public class PackingVisualizer extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        String problem = args.get(0).toLowerCase();
        String filename = args.get(1);

        // Load and fill the group depending on the problem
        if( problem == "pentomino"){
            this.objectGroup = loadPenominoProblem(filename);
        }
        else if(problem == "parcel"){
            this.objectGroup = loadParcelProblem(filename);
        }
        else{
    ...
    private Group loadParcelProblem(String filename){
        ParcelProblem parcelProblem = new ParcelProblem();
        // Get the argument and load from file
        parcelProblem.loadFromFile(filename);
...
//            phongMaterial.setDiffuseColor(Color.color(Math.random(), Math.random(), Math.random(), 1.0-transparency));
    ...
    private Group loadPenominoProblem(String filename){
        PentominoProblem pentominoProblem = new PentominoProblem();
        // Get the argument and load from file
        pentominoProblem.loadFromFile(filename);

        // Create the group to put all the objects in
        Group group = new Group();

        ArrayList<Pentomino> elements = pentominoProblem.getPentominos();
...
//            phongMaterial.setDiffuseColor(Color.color(Math.random(), Math.random(), Math.random(), 1.0-transparency));
            ...
            for (Coordinate coordinate:coordinates) {
                //TODO: create a mesh instead of boxes
    ...
    public void run(String problem, String filename) {
        //TODO: this only works for the pentomino problem yet
        launch(problem, filename);
    }
}
