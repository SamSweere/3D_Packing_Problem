...
public class Pentomino implements Comparable<Pentomino>{
    private Shape shape;
    ...
    public Pentomino(Shape shape, double value, Coordinate[] coordinates) {
        this.value = value;
        this.shape = shape;
    ...
    public Pentomino clone() {
        Pentomino pentomino = new Pentomino(this.shape, this.value);
        if(this.coordinates != null){
            pentomino.coordinates = Arrays.stream(this.coordinates).map(Coordinate::new).toArray(Coordinate[]::new);
        }
        return pentomino;
    }
    ...
    public String toString(){
        //TODO:this only show the coordinates now
        StringBuilder sb = new StringBuilder();
        sb.append("Pentominoe: ");
        for(int i = 0; i < this.coordinates.length; i++){
            sb.append(this.coordinates[i].toString());
    ...
    @Override
    public int compareTo(Pentomino pentomino) {
        if(this.value > pentomino.getValue()){
            return 1;
        }
        else if(this.value == pentomino.getValue()){
            return 0;
        }
        else{
            return -1;
